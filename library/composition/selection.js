...
const knownFacetPrefixes = ref([]);

export function configureSelection() {
  return {
    knownFacetPrefixes
  };
}
...
export function useSelection(initialValue = {}) {
  ...
  const selectionHandler = {
    get: function(target, prop, receiver) {
      if (target[prop] === undefined && typeof prop === 'string' &&
        (
          prop.substring(0, 1) != '_' ||
          prop.startsWith && knownFacetPrefixes.value.find(x => prop.startsWith(x))
        )
      ) {
        target[prop] = new Set();
      }
